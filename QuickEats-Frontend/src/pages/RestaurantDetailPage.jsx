...
const RestaurantDetailPage = () => {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      {restaurant && (
        <div className="bg-gradient-to-r from-slate-900 via-orange-950 to-slate-900 rounded-3xl p-8 text-white shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-2xl flex items-center gap-3 shrink-0">
            ...
            <div>
              <p className="text-lg font-black text-white">4.8 / 5.0</p>
      ...
      <div className="p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl flex items-center justify-between shadow-xs">
        <div className="flex items-center gap-2 text-xs font-bold text-amber-900">
          ...
          <span>Real-Time AI Market Pricing Engine Active: <span className="font-extrabold text-orange-700">{sampleSurge.label}</span></span>
        </div>
        <span className="bg-orange-600 text-white text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {menuItems.map((menu) => {
              ...
              return (
                <div
                  key={menu.id}
                  className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all flex justify-between items-center gap-4"
                >
                  <div className="space-y-1.5 flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-base text-slate-900">{menu.itemName}</h3>
                  ...
                  <div className="shrink-0">
                    {qty === 0 ? (
                      <button
                        ...
                        className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs rounded-xl shadow-md shadow-orange-600/20 transition-all flex items-center gap-1.5"
                      >
